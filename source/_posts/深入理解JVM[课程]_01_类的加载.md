...
#### <center><font color = "#36648B">✎</font><br/><font color = "#36648B">类加载的定义</font></center>
...
> 所以我们在编译完之后，删除某个类的.class文件，如果没有主动使用它，那么就不会报错。
...
- Java虚拟机自带的加载器
  ...
  - 系统（应用）类加载器（System）:也称为应用类加载器，它的父加载器为**扩展类加载器**。它从环境变量`classpath`或者系统属性`java.class.path`所指定的目录中加载类，它是**用户自定义的类加载器的默认父加载器**。系统类加载器是纯Java类，**是java.lang.ClassLoader类的子类**。
- 用户自定义的类加载器
